...
select L('
    for value in sqlite.urows("select num2 from numbers008") do
...
select L('
    ...
    sqlite.create_function("sub1", sub1, 0)
');
select sub1();

select L('
    local sub2 = function()
        for value in sqlite.urows("select sub1() from numbers008") do
            return tonumber(value)
            -- urows in unfinalized_statements
        end
    end
    sqlite.create_function("sub2", sub2, 0)
');
select sub2();
